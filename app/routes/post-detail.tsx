    ...
    <main className="mx-auto w-full max-w-5xl px-5 pb-20 pt-8 md:px-8 lg:pb-28 lg:pt-14">
      <article className="rounded-xl border border-line bg-surface px-5 py-8 shadow-soft md:px-10 md:py-12 lg:px-20 lg:py-16">
        <header className="mx-auto max-w-reading border-b border-line pb-8 text-left lg:pb-10">
          ...
          <h1 className="mt-5 font-display text-xl font-semibold leading-tight tracking-tight text-ink md:text-2xl lg:text-3xl">{post.title}</h1>
          {post.tags.length > 0 && <div className="mt-5 flex flex-wrap justify-start gap-3">{post.tags.map((tag) => <Link key={tag.slug} className="font-mono text-xs text-mint-strong hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent" to={`/tags/${tag.slug}`}>#{tag.name}</Link>)}</div>}
        </header>
        {post.renderedHtml ? (
          <div className="article-rich-text mx-auto mt-10 max-w-reading" dangerouslySetInnerHTML={{ __html: post.renderedHtml }} />
        ) : (
